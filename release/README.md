# `bted`'s Reproducible Build System

This package contains the build script that the `bted` project uses in order to
...
For example (taken from [f3ec130](https://github.com/bitweb-project/bted/commit/f3ec13030e4e828869954472cbc51ac36bee5c1d)):
```diff
...
1. `git clone https://github.com/bitweb-project/bted.git`
2. `cd bted`
3. `./release/release.sh <TAG> # <TAG> is the name of the next release/tag`

This will then create a directory of the form `bted-<TAG>` containing archives
...
Now that the directory `bted-<TAG>` is created, the manifest file needs to be
...
which roughly follow the format of [previous release notes](https://github.com/bitweb-project/bted/releases/tag/v0.20.1-beta).
...
When following the GitHub documentation, include every file in the `bted-<TAG>`
directory.
...
5. Ensure `go` is installed, matching the same version as noted in the release
   notes. 
6. Obtain a copy of `bted`'s source code with `git clone
   https://github.com/bitweb-project/bted` and checkout the source code of the
...
8. Extract the archive found in the `bted-<TAG>` directory created by the
   release script and recompute the `SHA256` hash of the release binaries (bted
